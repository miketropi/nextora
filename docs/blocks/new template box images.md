...
- Inspector Controls & Item Modal:
  - When `template === 'template4'`:
    ...
    - **Stage Max Width (Horizontal Gap)**: Added `stepHorizontalGap` (default `1140px`, range `800px`–`1400px`) in the Layout panel to control the container width and horizontal spread between zigzag cards.
    - **Sequential SVG Path Animation**: When `scrollAnimationStyle === 'sequential'`, the animated connecting S-curve path weaves dynamically from card to card via an SVG mask, drawing each dashed line segment just before revealing the corresponding step card, then seamlessly transitioning into the continuous marching dashed loop.
