...
The **Button Icon** block (`nextora/button-icon`) renders a theme CTA button with an optional **Lucide theme icon** or **uploaded image icon** beside the label — similar to WordPress **`core/button`**, extended with the icon system from [`nextora/icon`](../../blocks/icon/).
...
| Item | Value |
|------|--------|
| **Block name** | `nextora/button-icon` |
| **Title** | Button Icon |
| **Category** | `design` |
| **Text domain** | `nextora` |
| **PHP prefix** | `nextora_` |
...
| **Scaffold** | `npm run gen -- --name=button-icon --ns=nextora --category=design` |
...
| Use `nextora/button-icon` | Use `nextora/icon` instead |
|----------------------------|-----------------------------|
...
| Feature | `core/button` | `nextora/button-icon` |
|---------|---------------|------------------------|
...
| Attribute | Type | Default | Control | Description |
|-----------|------|---------|---------|-------------|
| `buttonStyle` | `string` | `"fill"` | SelectControl | `fill` (solid) or `outline` — maps to `nextora-button-icon--style-fill` / `--style-outline` |
...
Prefix: **`--nextora-button-icon-*`**

| Variable | Purpose |
|----------|---------|
| `--nextora-button-icon-radius` | Button border radius |
| `--nextora-button-icon-gap` | Icon–label gap |
| `--nextora-button-icon-bg` | Fill background override |
| `--nextora-button-icon-text` | Text + icon color override |
| `--nextora-button-icon-border` | Outline border color |
| `--nextora-button-icon-icon-radius` | Stacked/framed icon surface |
...
| Element | Class |
|---------|--------|
| Root | `wp-block-nextora-button-icon nextora-button-icon` |
| Alignment | `nextora-button-icon--align-{left\|center\|right}` |
| Button style | `nextora-button-icon--style-fill` / `--style-outline` |
| Link | `nextora-button-icon__button wp-element-button` |
| Label | `nextora-button-icon__label` |
| Icon wrap | `nextora-button-icon__icon` |
| Icon position | `nextora-button-icon__icon--left` / `--right` / `--only` |
| Icon surface | `nextora-button-icon__icon-surface` (stacked/framed) |
| Scroll off | `nextora-button-icon--scroll-off` |

Mirror Gutenberg: root may also include `wp-block-button` compatibility class **only if** needed for global button CSS — prefer scoped BEM under `.nextora-button-icon`.
...
```html
<div
  class="wp-block-nextora-button-icon nextora-button-icon nextora-button-icon--align-center nextora-button-icon--style-fill"
  style="--nextora-button-icon-radius:50px;--nextora-button-icon-gap:0.5rem;"
  data-nextora-scroll-reveal="1"
>
  <a
    href="https://example.com/donate"
    class="nextora-button-icon__button wp-element-button"
    target="_self"
  >
    <span class="nextora-button-icon__icon nextora-button-icon__icon--left" aria-hidden="true">
      <svg class="lucide lucide-heart" …>…</svg>
    </span>
    <span class="nextora-button-icon__label">Donate now</span>
  </a>
</div>
```
...
```html
<div class="wp-block-nextora-button-icon nextora-button-icon nextora-button-icon--style-outline …">
  <a
    href="https://example.com"
    class="nextora-button-icon__button wp-element-button"
    aria-label="Open menu"
  >
    <span class="nextora-button-icon__icon nextora-button-icon__icon--only" aria-hidden="true">
...
- Upload mode: `<img class="nextora-button-icon__img">` inside icon span.
...
- Map `iconStyle` stacked/framed → inner `nextora-button-icon__icon-surface` wrapper (same pattern as `nextora-icon__surface`)
...
| `false` | `data-nextora-scroll-animation-init="1"`, classes `nextora-button-icon--scroll-off nextora-scroll-animation--ready` — **no** `data-nextora-scroll-reveal` |

Use `nextora_icon_scroll_animation_enabled()` (shared) or duplicate as `nextora_button_icon_scroll_animation_enabled()` with identical `filter_var` logic.
...
- **RichText** for `text` inside the button preview (tagName `span`, class `nextora-button-icon__label`) — mirror `core/button` inline editing.
...
- Selector: `.wp-block-nextora-button-icon.nextora-button-icon`
...
- `.nextora-button-icon__button` — CTA tokens (padding, weight, radius, hover, `:focus-visible`)
...
| Focus | `:focus-visible` ring on `.nextora-button-icon__button` — match [`buttons.css`](../nextora/resources/css/modules/components/buttons.css) patterns |
...
npm run gen -- --name=button-icon --ns=nextora --category=design
...
| Rule | Detail |
|------|--------|
| **Namespace** | `nextora/button-icon` only |
| **Text domain** | `nextora` |
...
*Nextora — Button Icon Block (`nextora/button-icon`) agent specification*
