...
Built in **`inc/features/spotlight-search/modal-markup.php`** (`nextora_get_header_search_modal_markup()`), used by the **`nextora/spotlight-search`** block and optionally by the legacy PHP hook in **`register-hooks.php`**: same root + scrim + **one** `data-nextora-modal-surface` panel with `nextora-modal__surface--spotlight`. The dialog uses **`aria-label`** (search title string) and **`aria-describedby`** on the form hint (`{id}-spotlight-hint`) instead of a visible header row. **`nextora-modal__body`** wraps the search form; the close button stays **after** the body in the DOM and is absolutely positioned with top padding on the body so it does not cover the field. Full behavior, REST filters, and `data-*` hooks are documented in **[spotlight-search.md](./spotlight-search.md)**.
