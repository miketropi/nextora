...
| `nextora_header_search_modal_markup_args` | Merge class strings and labels (`title_text` → dialog `aria-label`, `spotlight_subtitle_text` still in args for child themes / filters though the default shell no longer prints a header row; `spotlight_modal_header_class`, `spotlight_title_class`, … for custom `nextora_header_search_modal_output`). `spotlight_close_wrap_class`, `trigger_class`, … Keep `nextora-modal` / `nextora-modal__surface` on the root and panel unless you replace `nextora_header_search_modal_output` entirely. |
