...
function initIn(container: Element | Document) {
  const roots = container.querySelectorAll<HTMLElement>('.nextora-sw');
  roots.forEach((root) => {
    if (root.dataset.swiperInited === '1' || root.dataset.swiperInitPending === '1') {
      return;
    }

    const el = root.querySelector<HTMLElement>('.nextora-sw__swiper');
    if (!el) {
      return;
    }
    ...
    const nextEl = root.querySelector<HTMLElement>('.nextora-sw__arrow--next');
    const prevEl = root.querySelector<HTMLElement>('.nextora-sw__arrow--prev');
    const paginationEl = root.querySelector<HTMLElement>('.nextora-sw__pagination');
    ...
    const canLoop = wantLoop && slideCount >= Math.max(2, spv + spg);
    ...
    const tryMount = (tick = 0): void => {
      ...
      root.classList.remove('nextora-sw--loading');
      root.classList.add('nextora-sw--ready');
    };
