/**
 * Scroll reveal for `nextora/icon` (front end).
 *
...
const BLOCK_SELECTOR = '.wp-block-nextora-icon.nextora-icon';
const BLOCK_INIT_ATTR = 'data-nextora-icon-scroll-init';
...
function optOutScrollAnimation( root: HTMLElement ): void {
	...
	root.classList.remove( 'nextora-icon--reveal-pending' );
	root.classList.add( 'nextora-scroll-animation--ready' );
	root.classList.add( 'nextora-icon--reveal-ready' );
...
function initReveal( root: HTMLElement ): void {
	...
	root.setAttribute( BLOCK_INIT_ATTR, '1' );
	root.classList.add( 'nextora-icon--reveal-pending' );
