...
/** Store theme preset slugs so CSS vars follow style variations. */
export function normalizeColorForStorage(value: string | undefined, palette: PaletteColor[]): string {
  if (!value) {
    return '';
  }
...
/** Value for PanelColorSettings — uses the active palette hex when possible. */
...
export function storedColorToCss(value: string): string {
  if (!value) {
    return '';
  }
