...
if ( ! function_exists( 'nextora_event_get_color_props' ) ) {
	/**
	 ...
	 */
	function nextora_event_get_color_props( string $color, string $type = 'color' ): array {
		$color = trim( $color );
		if ( '' === $color ) {
...
$title_color_props = nextora_event_get_color_props( $title_color_raw, 'color' );
...
$card_bg_props     = nextora_event_get_color_props( $card_bg_raw, 'background' );
$card_border_props = nextora_event_get_color_props( $card_border_raw, 'border' );
...
$date_props     = array(
	'bg'    => nextora_event_get_color_props( $date_bg_raw, 'background' ),
	'day'   => nextora_event_get_color_props( $date_day_raw, 'color' ),
	'month' => nextora_event_get_color_props( $date_month_raw, 'color' ),
);
...
$meta_props     = array(
	'text' => nextora_event_get_color_props( $meta_color_raw, 'color' ),
	'icon' => nextora_event_get_color_props( $meta_icon_raw, 'color' ),
);
...
$reg_bg_props     = nextora_event_get_color_props( $reg_bg_raw, 'background' );
$reg_text_props   = nextora_event_get_color_props( $reg_text_raw, 'color' );
$reg_border_props = nextora_event_get_color_props( $reg_border_raw, 'border' );
...
if ( ! function_exists( 'nextora_event_render_card' ) ) {
	 ...
	 * @param array<string, string>                $title_color_props Title color classes and styles.
	 * @param array<string, string>                $card_props        Card background and border classes and styles.
	 * @param array<string, string>                $reg_btn_props     Register button classes and styles.
	 * @param array<string, array<string, string>> $date_props        Date badge color props.
	 * @param array<string, array<string, string>> $meta_props        Meta text and icon color props.
	 */
	function nextora_event_render_card(
		...
		if ( '' !== trim( $event['location'] ) ) {
			$details[] = sprintf(
				...
				$meta_text_classes,
				$meta_text_style,
				nextora_event_detail_icon( 'map-pin', $icon_props ),
		...
		if ( '' !== trim( $event['time'] ) ) {
			$details[] = sprintf(
				...
				$meta_text_classes,
				$meta_text_style,
				nextora_event_detail_icon( 'clock', $icon_props ),
				esc_html( $event['time'] ),
			);
		}
		if ( '' !== trim( $event['price'] ) ) {
			$details[] = sprintf(
				...
				$meta_text_classes,
				$meta_text_style,
				nextora_event_detail_icon( 'ticket', $icon_props ),
...
if ( ! function_exists( 'nextora_event_render_template2_item' ) ) {
	/**
	 ...
	 * @param array<string, array<string, string>> $meta_props        Meta text and icon color props.
	 */
	function nextora_event_render_template2_item(
		array $event,
		string $placeholder_url,
		bool $show_register,
		string $default_register,
		...
		$details = array();
		if ( '' !== trim( $event['time'] ) ) {
			$details[] = sprintf( '<span class="nextora-event__template2-meta%1$s"%2$s>%3$s<span>%4$s</span></span>', $meta_text_class, $meta_text_style, nextora_event_detail_icon( 'clock', $icon_props ), esc_html( $event['time'] ) );
		}
		if ( '' !== trim( $event['location'] ) ) {
			$details[] = sprintf( '<span class="nextora-event__template2-meta%1$s"%2$s>%3$s<span>%4$s</span></span>', $meta_text_class, $meta_text_style, nextora_event_detail_icon( 'map-pin', $icon_props ), esc_html( $event['location'] ) );
		}
...
if ( ! function_exists( 'nextora_event_render_template3_item' ) ) {
	/**
	 ...
	 * @param array<string, array<string, string>> $meta_props        Meta text and icon color props.
	 */
	function nextora_event_render_template3_item(
		array $event,
		string $placeholder_url,
		bool $show_register,
		array $title_color_props = array(),
		...
		$meta_time = '' !== trim( $event['time'] )
			? sprintf( '<span class="%1$s"%2$s>%3$s%4$s</span>', esc_attr( trim( 'nextora-event__template3-meta-item' . $meta_text_class ) ), $meta_text_style, nextora_event_detail_icon( 'clock', $icon_props ), esc_html( $event['time'] ) )
			: '';
		$meta_location = '' !== trim( $event['location'] )
			? sprintf( '<span class="%1$s"%2$s>%3$s%4$s</span>', esc_attr( trim( 'nextora-event__template3-meta-item' . $meta_text_class ) ), $meta_text_style, nextora_event_detail_icon( 'map-pin', $icon_props ), esc_html( $event['location'] ) )
			: '';
