/**
 * Scroll reveal for `nextora/button-icon` (front end).
 *
...
const BLOCK_SELECTOR = '.wp-block-nextora-button-icon.nextora-button-icon';
const BLOCK_INIT_ATTR = 'data-nextora-button-icon-scroll-init';
...
function optOutScrollAnimation( root: HTMLElement ): void {
	...
	root.classList.remove( 'nextora-button-icon--reveal-pending' );
	root.classList.add( 'nextora-scroll-animation--ready' );
	root.classList.add( 'nextora-button-icon--reveal-ready' );
...
function initReveal( root: HTMLElement ): void {
	...
	root.setAttribute( BLOCK_INIT_ATTR, '1' );
	root.classList.add( 'nextora-button-icon--reveal-pending' );
