<?php
/**
 * Button Icon — container block render.
 *
 * @package Nextora
 *
...
if ( ! function_exists( 'nextora_button_icon_enqueue_view_script' ) ) {
	/**
	 * Queue block view script when scroll reveal may run.
	 */
	function nextora_button_icon_enqueue_view_script(): void {
		if ( is_admin() ) {
			return;
		}

		$registry = WP_Block_Type_Registry::get_instance()->get_registered( 'nextora/button-icon' );
...
if ( ! function_exists( 'nextora_button_icon_strip_inner_wrapper' ) ) {
	/**
	 * Remove the saved inner wrapper div from InnerBlocks markup.
	 *
	 * @param string $html Saved inner HTML.
	 */
	function nextora_button_icon_strip_inner_wrapper( string $html ): string {
		$html = trim( $html );
		if ( '' === $html || 0 !== strpos( $html, '<div' ) ) {
			return $html;
		}

		if ( preg_match( '/^<div[^>]*class="[^"]*nextora-button-icon__inner[^"]*"[^>]*>(.*)<\/div>\s*$/s', $html, $matches ) ) {
...
if ( $block instanceof WP_Block && $block->inner_blocks !== null && count( $block->inner_blocks ) > 0 ) {
	...
	$buttons_html = nextora_button_icon_strip_inner_wrapper( $content );
}
...
$wrapper_classes = array(
	'nextora-button-icon',
	'nextora-button-icon--align-' . sanitize_html_class( $button_align ),
);

if ( ! $enable_scroll ) {
	$wrapper_classes[] = 'nextora-button-icon--scroll-off';
...
nextora_button_icon_enqueue_view_script();
...
<div <?php echo $wrapper; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped?>>
	<div class="nextora-button-icon__buttons">
