"use strict";
(() => {
  // resources/ts/lib/article-share.ts
  ...
  function setVisibleFeedback(el, message) {
    if (!el) {
      return;
    }
    el.textContent = message;
  ...
  function initArticleShare() {
    const roots = document.querySelectorAll("[data-nextora-article-share]");
    if (roots.length === 0) {
      return;
    }
    const i18n = window.nextoraArticleShare ?? {};
    for (const root2 of roots) {
      const btn = root2.querySelector("[data-nextora-copy-url]");
      const status = root2.querySelector("[data-nextora-copy-status]");
      const feedback = root2.querySelector("[data-nextora-copy-feedback]");
      if (!btn) {
        continue;
      }
...
${nextLine.slice(indentLevel + 2)}`;
  ...
  var HEADER_SEL = ".nextora-header-block--sticky-scroll-up";
  ...
  function bindSpotlightForm(form, config) {
    ...
    const runFetch = async (q) => {
      ...
      const { signal } = abort;
      setLoading(true);
      setStatus2(config.loading, false);
